...
export function TopEpisodes() {
  ...
  return (
    <section className="py-16 bg-[var(--bg)] relative overflow-hidden">
      {/* Decorative diagonal stripe - fades at ~75% */}
      <div 
        ...
        style={{
          ...
          width: '120%',
          height: '180px',
          background: 'linear-gradient(to right, #d5d9e0 0%, #d5d9e0 55%, transparent 75%)',
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="mb-8">
          <div className="flex items-center justify-center mb-4">
            <RibbonHeader text="AUDIENCE PICKS" />
          </div>
        </div>
        <div className="relative max-w-7xl mx-auto">
          {/* Left arrow overlay - half on carousel, half off */}
          <button
            onClick={() => {
              api?.scrollPrev()
              resetAutoplay()
          ...
          {/* Right arrow overlay - half on carousel, half off */}
          <button
            onClick={() => {
              api?.scrollNext()
              resetAutoplay()
